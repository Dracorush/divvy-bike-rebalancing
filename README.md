...
But we can constrain it. There exists a range of valid starting values `[L, U]` such that if the curve starts anywhere in that range, it never dips below 0 or rises above `dpcapacity` at any point during the day. Start too low and the curve goes negative at some point — physically impossible. Start too high and it overflows capacity — also impossible.
...
All trips are grouped into hourly buckets using `EXTRACT(hour FROM starttime)` and `EXTRACT(hour FROM stoptime)`. This gives us up to 24 slots per station per day — hour 0 through hour 23.
...
We compute the running total of `hourly_net_flow` across the 24 hours of the day in order. This gives us the shape of the inventory curve relative to wherever it started — how much it moved up or down from its starting point by each hour.
...
Days with zero trips get `min_cumulative_flow = 0` and `max_cumulative_flow = 0` — the curve never moved.
...
**Lower bound L:**
The curve drops by at most `-min_cumulative_flow` from its starting point during the day. If the starting inventory were any lower than this, the curve would go negative at some point — physically impossible. So the starting inventory must be at least `-min_cumulative_flow`.
...
**Station identity:**
- `station_id` as a categorical feature — lets the model learn station-specific patterns without manual encoding
...
The secondary metric is **conditional efficiency** — for covered predictions only, how close was the prediction to the midpoint of the interval:
```
...
- Each station is connected to its 8 nearest neighbors by Haversine distance computed from `latitude_start`, `longitude_start` — these are the edges bikes can travel along
